...
\begin{document}
	...
	\begin{frame}{Safe Sources and Safe Sinks}
		...
		\only<3,4>{\begin{itemize}
			\item For $\T \in \mathcal{M}_{+}$, $t$ is a safe source in $\T$ if :\begin{itemize}
				\item[a]<3,4>{For every $t\in{X}\in\mathcal{T}_{-}$, we have $\color{red}\T\subsetneq X$.}
				\item[b]<4>{For every $t\in{X}\in\mathcal{D}_{-}$ such that $\T\setminus{X}\not=\varnothing$, there exists $Y\in\mathcal{T}_{-}$ such that $t\not\in{Y}\subsetneq{X}$.}
			\end{itemize}
		\end{itemize}
		\begin{columns}
			\begin{column}{0.45\textwidth}
