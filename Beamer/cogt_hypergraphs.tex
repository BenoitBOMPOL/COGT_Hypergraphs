...
\begin{document}
	...
	\begin{frame}{State of the art, goal of the article}
		\visible<1->{
			\begin{tcolorbox}[colback=richcarmine!5!white,colframe=richcarmine!75!black,title=Nash-Williams (1960)]
				$G$ is a $2k$-edge connected undirected graph $\Leftrightarrow$ it admits a $k$-arc connected orientation.
			\end{tcolorbox}
		}
		...
		\begin{columns}
			\begin{column}{0.5\textwidth}
				\begin{figure}[H]
					...
					\begin{tikzpicture}
						...
						\fill (A) circle (2pt); \visible<1>{\node[above right=2pt] at (A) {$A$};} \visible<2>{\node[above right=2pt] at (A) {$A = r$};} \visible<2>{\fill[alizarin] (A) circle (4pt);} \visible<5>{\fill[caribbeangreen] (A) circle (3 pt);} \visible<13-18>{\fill[atomictangerine] (A) circle (3 pt);}
						\fill (B) circle (2pt); \node[right=2pt] at (B) {$B$};
			...
			\begin{column}{0.5\textwidth}
				\begin{enumerate}
					...
					\item <alert@5,9,13> Find an admissible $(s, t)$-hyperpath in $R$ to reorient
	...
	\begin{frame}{Finding \textit{admissible} $(s, t)$-hyperpaths in $R\in\mathcal{R}$}
		...
		\begin{itemize}
			...
			\item[3.] Let $\vec{\mathcal{H}'}$ the hypergraph obtained after reorientation of $P$.
		\end{itemize}
	...
	\begin{frame}{Proof of Lemma 13}
		...
		\only<3>{
			...
			\begin{itemize}
				...
				\item[.] $S\in{\color{atomictangerine}\mathcal{T}_{-}}, S\subseteq R\cap{X}$. Since $t\in R\setminus X, X\cap R\subsetneq R$.
