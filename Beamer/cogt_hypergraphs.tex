...
\begin{document}
	...
	\begin{frame}{Hyperarc-connectivity and $(k, k)$-partition connected hypergraphs}
		\begin{itemize}
			...
			\visible<2->{\item The previous orientation given was 2-hyperarc-connected.}
		\end{itemize}
		
		\begin{itemize}
			\visible<3->{\item  Let $\mathcal{P}$ be a partition of $V$ :}
			\visible<3->{\item $e_{\mathcal{H}}(\mathcal{P})$ is the number of hyperedges intersecting at least 2 elements of $\mathcal{P}$}
			\visible<4->{\item $\mathcal{H}$ is $(k,k)$-partition-connected, if : \begin{itemize}
					\item $\forall\mathcal{P}, e_{\mathcal{H}}(\mathcal{P}) \geq k \times |\mathcal{P}| $
			\end{itemize}}
		\end{itemize}
		\visible<5->{We use a result of Frank : $\mathcal{H}$ is $(k, k)$-partition-connected if and only if it admits a $k$-hyperarc-connected orientation.}
	\end{frame}
	...
	\begin{frame}{Tight and Dangerous sets}
			...
			\begin{column}{0.45\textwidth}
				\begin{figure}[H]
					\begin{tikzpicture}
						...
						\visible<2>{\draw[ultra thick, ->, caribbeangreen] (V) to [out=-90, in=90] (v);}
						\draw[ultra thick, ->, caribbeangreen] (W) to [out=-120, in=60] (w);
						
						\visible<-4>{\draw[thin] (0,0) circle (\mid);}
						\visible<4->{\draw[thin, dashed] (0,0) circle (\smol);}
									
					\end{tikzpicture}

					\caption*{\only<4->{Minimal }In-\only<1,3->{Tight}\only<2>{Dangerous} sets}
				\end{figure}
			\end{column}
			\hfill\vrule\hfill
			\begin{column}{0.45\textwidth}
				\begin{figure}[H]
					\begin{tikzpicture}
						...
						\visible<2>{\draw[ultra thick, <-, atomictangerine] (V) to [out=-90, in=90] (v);}
						\draw[ultra thick, <-, atomictangerine] (W) to [out=-120, in=60] (w);
						
						\visible<-4>{\draw[thin] (0,0) circle (\mid);}
						\visible<4->{\draw[thin, dashed] (0,0) circle (\smol);}				
					\end{tikzpicture}
					\caption*{\only<4->{Minimal }Out-\only<1,3->{Tight}\only<2>{Dangerous} sets}
				\end{figure}
			\end{column}
		\end{columns}
		...
		\only<1->{
			\begin{itemize}
				...
				\item<2> ${\color{caribbeangreen}\mathcal{D}_{-}} = \{X \subseteq V - r, d^{-}(X) = k + 1\}$ 
				\item<2> ${\color{atomictangerine}\mathcal{D}_{+}} = \{X \subseteq V - r, d^{+}(X) = k + 1\}$
				\item<4-> ${\color{caribbeangreen}\mathcal{M}_{-}}$ : Inclusion-wise minimal members of ${\color{caribbeangreen}\mathcal{T}_{-}}$
				\item<4-> ${\color{atomictangerine}\mathcal{M}_{+}}$ : Inclusion-wise minimal members of ${\color{atomictangerine}\mathcal{T}_{+}}$
				\item<4-> $\mathcal{M}$ : Inclusion-wise minimal members of $\mathcal{M}_{-}\cup\mathcal{M}_{+}$
			\end{itemize}
		}
	...
	\begin{frame}{Towards hyperarc connectivity augmentation}
		...
		\only<2->{
			\begin{block}{Proof of Lemma 13}
				...
				\begin{itemize}
					\item[\textbf<3>{a.}] $s\in X, t\not\in X, d^{+}(X) = k$, i.e. $s\in X, t\not\in X, X\in{\color{atomictangerine}\mathcal{T}_{+}}$.{\begin{itemize}
						\item[\textbf<3>{a1.}] $R\in\mathcal{R}\cap{\color{caribbeangreen}\mathcal{T}_{-}}$
						\item[\textbf<3>{a2.}] $R\in\mathcal{R}\cap{\color{atomictangerine}\mathcal{T}_{+}}$ 
					\end{itemize}}
	...
	\begin{frame}{Proof of Lemma 13}
		...
		\only<3>{
			...
			\begin{itemize}
				...
				\item[.] $S\in{\color{atomictangerine}\mathcal{T}_{-}}, S\subseteq R\cap{X}$. Since $r\in R\setminus X, X\cap R\subsetneq R$.
		...
		\only<-3>{
			\begin{alertblock}{Final words}
				\begin{itemize}[<+->]
					...
					\item Extensions of \textsf{Theorem 7} (main result) : Starting without conditions on $\lambda(\vec{\mathcal{H}})$.
				\end{itemize}
			\end{alertblock}
		}
