...
\begin{document}
	...
	\begin{frame}{Finding \textit{admissible} $(s, t)$-hyperpaths in $R$}
		\begin{itemize}
			...
			\begin{itemize}
				\item[1.] $s$ is a safe source in $S\subseteq{R}$, $t$ is a safe sink in $T\subseteq{R}$.\begin{itemize}
					\item The hyperpath obtained does not leave given sets.
				\end{itemize}
				\item[2.] Reorient each hyperarc, \textbf{one by one}, does not decrease the hyperarc-connectivity.{\begin{itemize}
					\item At the end, we will prove part of this statement.
				\end{itemize}}
		...
		\begin{itemize}
			...
			\item ${\color{atomictangerine}\mathcal{M}_{+}}$ : Inclusion-wise minimal members of ${\color{atomictangerine}\mathcal{T}_{+}}$
