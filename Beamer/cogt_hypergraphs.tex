		...
		\begin{block}{Proof of {\color{majorelleblue}Lemma 12(a)}}
			\begin{itemize}[<+->]
				...
				\item We have the following inequalities{\begin{itemize}[<+->]
					\item $d^{+}(Q^{s}_{+}) \geq d^{+}(Z)$
					\item $d^{+}(Z) \geq k$, as $\mathcal{H}$ is $k$-hyperarc-connected.
					\item $k = d^{+}(Q^{s}_{+})$ by definition.
				\end{itemize}}
				\item We can deduce that $d^{+}(Z) = k$, which automatically implies that $Z\in{\color{atomictangerine}\mathcal{T}_{+}}$.
